...
counts <- read_tsv("counts.txt", skip = 1) %>%
...
condition <- c("C", "T", "V", "C", "C", "T", "T", "V", "V")
dim(counts)

# Filter out genes expressed below cutoff
totalexp <- rowSums(counts[,7:15])
hist(totalexp)
...
# Plot MDS
samples <- c("C1","T1", "V1", "C2", "C3", "T2", "T3", "V2", "V3")
